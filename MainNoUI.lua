...
--== External Config / Headless flag
local _MEOWY_EXTERNAL = rawget(getgenv(), "MEOWYConfig") or {}
local _NO_UI = _MEOWY_EXTERNAL.NoUI == true
if _NO_UI then
    print("[MEOWY] Running in headless mode (No UI).")
end
...
if not _NO_UI then
    ...
    -- Lightweight stubs เพื่อไม่ให้ส่วนอื่นล้ม
...
local fishingState = { lockedPos = nil, didFocus = false, useEvent = false }
...
local Configuration = {
    ...
    Event = { AutoClaim=false, AutoClaim_Delay=3, AutoLottery=false, AutoLottery_Delay=3,AutoLike=false, },
...
if _NO_UI then
    Options = setmetatable({}, {
        __index = function()
            -- คืนอ็อบเจ็กต์ที่มี :SetValue() เพื่อ no-op การสั่งปิด/เปิด toggle จากโค้ดรันไทม์
            return { SetValue = function() end }
        end
    })
end
-- Merge external configuration (optional)
...
pcall(function()
    if _MEOWY_EXTERNAL and next(_MEOWY_EXTERNAL) then
        -- ไม่ต้องเอา key NoUI ใส่ใน Configuration
        local copy = table.clone(_MEOWY_EXTERNAL)
        copy.NoUI = nil
        _deepmerge(Configuration, copy)
    end
end)
...
--==============================================================
-- ใช้ได้ทั้งกับ Pet/Egg ที่เป็น Model หรือ BasePart
local function getRoot(inst)
    if not inst then return nil end
    if inst:IsA("BasePart") then
        return inst
    elseif inst:IsA("Model") then
        -- อย่าให้ regex มาแทนบรรทัดนี้เป็น getRoot(inst) เด็ดขาด
        local pp = inst.PrimaryPart
        if pp then return pp end

        -- เผื่อไม่มี PrimaryPart
        local rp = inst:FindFirstChild("RootPart")
        if rp and rp:IsA("BasePart") then return rp end

        -- fallback: หา BasePart ตัวแรกในโมเดล
...
local function Grid_OccupiedKeys()
    ...
    for _, model in ipairs(BlockFolder:GetChildren()) do
        ...
        if OwnedEggData and OwnedEggData:FindFirstChild(model.Name) then
            keys[Grid_keyXZ(getPivotCF(model).Position.X, getPivotCF(model).Position.Z)] = true
        end
    end
    return keys
end
...
--== Pet Management
local function _isOwnedPetModel(m)
    if not m then return false end
    -- เดิมรับเฉพาะ Model → ปรับให้รับ BasePart ด้วย
    local isPetInstance = m:IsA("Model") or m:IsA("BasePart")
    if not isPetInstance then return false end
    return m:GetAttribute("UserId") == PlayerUserID
end
...
local function _buildOwnedPetEntry(pet)
    local petUID = nil
    if pet then
        petUID = pet:GetAttribute("UID") or pet:GetAttribute("Uid") or pet:GetAttribute("uid")
                 or (pet:FindFirstChild("UID") and pet.UID.Value)
                 or pet.Name
    end
    petUID = tostring(petUID or "")

    if petUID == "" then return end

    local root = getRoot(pet)
    local _cashTxtRef
    ...
    local petType  = root and root:GetAttribute("Type") or nil
    local petMut   = root and root:GetAttribute("Mutate") or nil
    local isBig    = root and (root:GetAttribute("BigValue") ~= nil) or false
    local remoteRE = root and root:FindFirstChild("RE", true) or nil

    ------------------------------------------------------------------------
    -- DI node ใน PlayerGui: Data/Pets/<UID>/DI
    ------------------------------------------------------------------------
    local pg    = Player:FindFirstChild("PlayerGui")
    local data  = pg and pg:FindFirstChild("Data")
    local petsN = data and data:FindFirstChild("Pets")
    local owned = petsN and petsN:FindFirstChild(petUID)

    local diNode = owned and owned:FindFirstChild("DI") or nil

    ------------------------------------------------------------------------
    -- ตัดสินใจเรื่อง GridCoord และสถานะ BlockedF
    --  - ถ้า F=1  -> ไม่เซ็ต GridCoord (ไม่จองช่อง)
    --  - ถ้า F!=1 -> ใช้ X/Y/Z ถ้ามีเป็นตัวเลขครบ
    ------------------------------------------------------------------------
    local blockedF = false
    local GridCoord = nil
    if diNode then
        local f = diNode:GetAttribute("F")
        blockedF = (f == 1 or f == "1" or f == true)

        if not blockedF then
            local dx, dy, dz = diNode:GetAttribute("X"), diNode:GetAttribute("Y"), diNode:GetAttribute("Z")
            if typeof(dx) == "number" and typeof(dy) == "number" and typeof(dz) == "number" then
                GridCoord = Vector3.new(dx, dy, dz)
            end
        end 
    end

    ------------------------------------------------------------------------
    -- สร้าง/อัปเดต entry ใน OwnedPets
    ------------------------------------------------------------------------
    OwnedPets[petUID] = setmetatable({
        ...
        RE        = remoteRE,
        IsBig     = isBig,
        GridCoord = GridCoord,  -- จะเป็น nil ถ้า F=1 ⇒ ไม่จองพื้นที่
        BlockedF  = blockedF,   -- ใช้ guard ตอนเลือก/แทนที่
    ...
    ------------------------------------------------------------------------
    -- ผูกอัปเดตสด: เมื่อ DI.F / X / Y / Z เปลี่ยน
    --  - F เปลี่ยนเป็น 1  -> เคลียร์ GridCoord (ปลดจอง)
    --  - F กลับเป็น 0     -> ลองอ่าน X/Y/Z ตั้ง GridCoord ใหม่ ถ้าครบ
    ------------------------------------------------------------------------
    if diNode then
        table.insert(EnvirontmentConnections,
            diNode:GetAttributeChangedSignal("F"):Connect(function()
                local entry = OwnedPets[petUID]
                if not entry then return end

                local f2 = diNode:GetAttribute("F")
                local v  = (f2 == 1 or f2 == "1" or f2 == true)
                entry.BlockedF = v
                if v then
                    entry.GridCoord = nil  -- ปลดจองทันที
                else
                    local dx, dy, dz = diNode:GetAttribute("X"), diNode:GetAttribute("Y"), diNode:GetAttribute("Z")
                    if typeof(dx) == "number" and typeof(dy) == "number" and typeof(dz) == "number" then
                        entry.GridCoord = Vector3.new(dx, dy, dz)
                    else
                        entry.GridCoord = nil
                    end
                end
        ...
        -- ถ้าพิกัดเปลี่ยน ให้สะท้อนทันที (เฉพาะเมื่อยังไม่ถูกบล็อก)
        local function _onXYZChanged()
            local entry = OwnedPets[petUID]
            if not entry or entry.BlockedF then return end
            local dx, dy, dz = diNode:GetAttribute("X"), diNode:GetAttribute("Y"), diNode:GetAttribute("Z")
            if typeof(dx) == "number" and typeof(dy) == "number" and typeof(dz) == "number" then
                entry.GridCoord = Vector3.new(dx, dy, dz)
            else
        ...
        table.insert(EnvirontmentConnections, diNode:GetAttributeChangedSignal("X"):Connect(_onXYZChanged))
        table.insert(EnvirontmentConnections, diNode:GetAttributeChangedSignal("Y"):Connect(_onXYZChanged))
        table.insert(EnvirontmentConnections, diNode:GetAttributeChangedSignal("Z"):Connect(_onXYZChanged))
    end
end
...
for _, pet in pairs(Pet_Folder:GetChildren()) do
    task.spawn(_buildOwnedPetEntry, pet, tostring(pet))
end
...
function getBigPetSlotMapping()
    ...
    for _, petModel in ipairs(workspace.Pets:GetChildren()) do
        if petModel:GetAttribute("UserId") == PlayerUserID then
            local rootPart = petModel
            if rootPart and rootPart:FindFirstChild("GUI/BigPetGUI") then
                table.insert(myPlacedBigPets, { UID = petModel.Name, Position = getPivotCF(petModel).Position })
            end
        end
    end
...
local function collectAllOnce()
    ...
    pcall(function()
        Fluent:Notify({ Title = "Get All Money", Content = ("Claimed from %d pets."):format(n), Duration = 4 })
    end)
end
...
local SmartFeedConfig = {
    ...
    { Fruit = "Pumpkin", UnlockType = "MUTATION", UnlockTarget = "Halloween", Slots={1,2,3} },  
    { Fruit = "CandyCorn", UnlockType = "PET", UnlockTarget = {"Bat","BlackCat","CheshireCat","Crow","Centaur","SpiderHalloween","SkeletonHorse"}, Slots = {1,2} },
...
function openBigPetUIForSlot(slotNumber)
    ...
    local pivotCFrame = getPivotCF(switchModel)
...
--==============================================================
    local function _getEventFishScope()
        local fp = workspace:FindFirstChild("FishPoints")
        if not fp then return nil end
        for _, child in ipairs(fp:GetChildren()) do
            -- รองรับชื่อแบบ FishPoint, FishPoint1..6 หรือโหนดอื่นๆที่ขึ้นต้น FishPoint
            if tostring(child.Name):match("^FishPoint") then
                local fx = child:FindFirstChild("FX_Fish_Special")
                local scope = fx and fx:FindFirstChild("Scope")
                if scope and scope:IsA("BasePart") and scope.Parent and fx.Parent then
                    return scope
                end
            end
        end
    ...
    local function TeleportToWorldPos(pos, yOffset)
        local char = Players.LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if not (hrp and pos) then return false end
        yOffset = tonumber(yOffset) or 5
        local dest = pos + Vector3.new(0, yOffset, 0)
        hrp.CFrame = CFrame.new(dest, pos)
        return true
    end
    ...
    local function runAutoFishing(tok)
        ...
        while tok.alive do
        local scope = _getEventFishScope()
        if scope then
            -- ใช้จุดกิจกรรม
            local target = scope.Position
            if (not fishingState.lockedPos) or (fishingState.lockedPos - (target + Vector3.new(0,10,0))).Magnitude > 3 or not fishingState.useEvent then
                TeleportToWorldPos(target, 6)
                fishingState.lockedPos = target + Vector3.new(0, 10, 0) -- จุดที่เราจะ "Throw"
                fishingState.useEvent  = true
                dprint("[AutoFishing] Event scope detected at:", fishingState.lockedPos)
            end
        else
            if (not fishingState.lockedPos) or fishingState.useEvent then
                fishingState.lockedPos = getFishingThrowPos()
                fishingState.useEvent  = false
                dprint("[AutoFishing] Event scope not found. Fallback to default pos:", fishingState.lockedPos)
            end
        end

        if not fishingState.lockedPos then
            warn("[AutoFishing] ไม่พบตำแหน่งสำหรับ Throw! กำลังหยุด...")
            pcall(function() Options["AutoFishingToggle"]:SetValue(false) end)
            break
        end
        ...
        for _, egg in pairs(OwnedEggData:GetChildren()) do
            ...
            if di and egg:GetAttribute("D") and (ServerTime.Value >= egg:GetAttribute("D")) then
                if (wantArea == "Any") or (eggArea(egg) == wantArea) then
                    ...
                    local RootPart = getRoot(EggModel)
...
--==============================================================
--== Auto Like Helpers & Runner (รองรับ Season1/Season2)
local likedUserIds = {}

local function _getLikeProgress()
    local pg = Player:FindFirstChild("PlayerGui")
    local data = pg and pg:FindFirstChild("Data")
    ...
    local dailyComplete = (likes >= 3) or ((tonumber(season:GetAttribute("CC_DailyTask2")) or 0) == 1)
    local weeklyComplete = (weeklyLikes >= 20)
    return likes, dailyComplete, weeklyLikes, weeklyComplete
end

local function _pickOtherUserId()
    local me = Player
    local candidates = {}
...
local function runAutoLike(tok)
    likedUserIds = {} -- reset ต่อรอบ
    while tok.alive do
        local likes, dailyDone, weeklyLikes, weeklyDone = _getLikeProgress()

        -- หยุดเมื่อบรรลุทั้งรายวัน (>=3) และรายสัปดาห์ (>=20)
        if dailyDone and weeklyDone then
            dprint(("[AutoLike] Done. Daily=%d, Weekly=%d"):format(likes, weeklyLikes))
            break
        end
        ...
        if targetId then
            ...
            -- ไม่มีเป้าหมายในเซิร์ฟเวอร์นี้ รอสั้น ๆ แล้ววนใหม่
            task.wait(2.0)
        end
    end
end
...
local function __findWorstPlacedPetInArea(areaWant)
    ...
    for uid, petData in pairs(OwnedPets) do
        if petData and not petData.IsBig and (areaWant == "Any" or petArea(uid) == areaWant) and not petData.BlockedF then
    ...
    while tok.alive do
        ...
        if #freeLandTiles > 0 or #freeWaterTiles > 0 then
            ...
            for _, uid in ipairs(tempUidsToPlace) do
                ...
                -- ⛔ ถ้า DI.F = 1 ให้ข้ามการวางตัวนี้
                do
                    local node = OwnedPetData and OwnedPetData:FindFirstChild(uid)
                    local di   = node and node:FindFirstChild("DI")
                    local f    = di and di:GetAttribute("F")
                    if f == 1 or f == "1" or f == true then
                        -- เอาออกจากคิว uidsToPlace ด้วย (กันวนซ้ำ)
        ...
        local function attemptReplacement(area)
            ...
            if worstUid then
                ...
                for _, uid in ipairs(uidsToPlace) do
                    -- เช็ก DI.F = 1 → ข้ามด้วยการห่อเงื่อนไข (ไม่ใช้ continue)
                    local isBlocked = false
                    do
                        local node = OwnedPetData and OwnedPetData:FindFirstChild(uid)
                        local di   = node and node:FindFirstChild("DI")
                        local f    = di and di:GetAttribute("F")
                        isBlocked  = (f == 1 or f == "1" or f == true)
                    end

                    if (not isBlocked) and (GetPetHabitat(_petTypeByUID(uid)) == area) then
...
-- Open to the Home tab by default
if not _NO_UI then
    --==============================================================
--                       UI DEFINITION
--==============================================================
...
Tabs.Main:AddButton({
    ...
    Description = "Get Money from all pets once",
    Callback = function()
        collectAllOnce()
    end
})
...
Tabs.Main:AddToggle("Auto Like Quest",{
    Title = "Auto Like Quest",
    Default = false,
    Callback = function(v)
        Configuration.Event.AutoLike = v
        if IsLoadingConfig then return end
        if v then
            TaskMgr.start("AutoLike", runAutoLike)
        else
            TaskMgr.stop("AutoLike")
        end
    end
})
...
Window.RefreshStatus = function()
  ...
  local lines = {
    ...
    string.format("%s Auto Like", mark("AutoLike")),
...
task.spawn(function()
    ...
    SaveManager:LoadAutoloadConfig()   -- ถ้าเป็น async ให้ดูหมายเหตุด้านล่าง

    updateBigPetSlots()
    task.defer(function()
        if ResultPara and ResultPara.SetDesc then
            ResultPara:SetDesc(renderSummary())
        end
    end)
    ...
    -- 5) บูตงานตามค่า (สำคัญมาก)
    Fluent:Notify({ Title = "System", Content = "Starting enabled tasks...", Duration = 3 })
    task.defer(function()
        _autostart()  
    end)

    if not _NO_UI and Window and Window.RefreshStatus then
        task.spawn(function()
...
Window:SelectTab(Home) 
    ...
    -- เก็บอ้างอิงไว้ใช้ล้างก่อนรันใหม่
    getgenv().MeowyBuildAZoo = Window
else
    -- Headless placeholder สำหรับรีรัน:Destroy()
    local _headless = {}
    function _headless:Destroy()
        RunningEnvirontments = false
        TaskMgr.stopAll()
        -- ย้อนค่าปรับแต่งภาพให้กลับมา
        ApplyHidePets(false); ApplyHideEggs(false); ApplyHideEffects(false); ApplyHideGameUI(false); Perf_Set3DEnabled(true)
        pcall(function()
